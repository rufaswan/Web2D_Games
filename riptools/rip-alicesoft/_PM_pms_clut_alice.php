...
require 'common.inc';
//////////////////////////////
function data_pms16( string &$file, array &$pms, int $st ) : string
{
	$data = [];
	for ( $y=0; $y < $pms['h']; $y++ )
	{
	...
	foreach ( $data as $k => $v )
	{
		...
		$g = ($v >> 3) & 0xfc; // <<  5 >> 8
		$b = ($v << 3) & 0xf8; // << 11 >> 8
		$data[$k] = [ chr($r) , chr($g) , chr($b) ];
	}
	return $data;
}
//////////////////////////////
function data_pms8( string &$file, array &$pms, int $st ) : string
{
	$data = [];
	for ( $y=0; $y < $pms['h']; $y++ )
	{
...
function clut_pms8( string &$file, array &$pms, int $st ) : string
{
	$clut = '';
	for ( $i=0; $i < 0x100; $i++ )
	{
...
//////////////////////////////////////////////////
function pms2clut( string $fname ) : void
{
	...
	if ( $pms['x'] !== 0 )  $fn .= '.x'.$pms['x'];
	if ( $pms['y'] !== 0 )  $fn .= '.y'.$pms['y'];
	...
	switch ( $pms['bpp'] )
	{
		case 8:
			...
			$clut = 'CLUT';
			$clut .= tool::chr(256 , 4);
			$clut .= tool::chr($pms['w'] , 4);
			$clut .= tool::chr($pms['h'] , 4);

			$clut .= clut_pms8($file , $pms, $pms['pal']);
			$clut .= data_pms8($file , $pms, $pms['dat']);

			file_put_contents("$fn.clut", $clut);
			return;
		...
		case 16:
			...
			$rgba = 'RGBA';
			$rgba .= tool::chr($pms['w'] , 4);
			$rgba .= tool::chr($pms['h'] , 4);
			...
			$len = $pms['w'] * $pms['h'];
			while ( strlen($pix) < ($len*3) )
				$pix .= ZERO;
			while ( strlen($alp) < $len )
				$alp .= BYTE;
			...
			file_put_contents("$fn.rgba", $rgba);
			return;
