...
so=(
	...
	"$PWD"/lib32:$libex
	"$PWD"/lib
	"$PWD"/lib/i386-linux-gnu
	"$PWD"/usr/lib
	"$PWD"/usr/lib/i386-linux-gnu
)
export LD_LIBRARY_PATH=$(IFS=\:; echo "${so[*]}")
...
so=(
	...
	"$PWD"/lib32/gtk-2.0
	"$PWD"/usr/lib/gtk-2.0
	"$PWD"/usr/lib/i386-linux-gnu/gtk-2.0
)
export GTK_PATH=$(IFS=\:; echo "${so[*]}")
echo "== GTK_PATH = $so"
...
so=(
	...
	"$PWD"/lib32/dri
	"$PWD"/usr/lib/dri
	"$PWD"/usr/lib/i386-linux-gnu/dri
)
export LIBGL_DRIVERS_PATH=$(IFS=\:; echo "${so[*]}")
