...
// echo -n hello | wc -c # =5
// wc -c <<< hello       # =6 , auto append newline
foreach ( $str as $s )
{
	echo "STR = '$s'\n\n";
...
$md5    = hmac::md5   ($k, $v);
$sha1   = hmac::sha1  ($k, $v);
$sha256 = hmac::sha256($k, $v);
$sha512 = hmac::sha512($k, $v);
	printf("HMAC  md5   %s\n", bin2hex($md5   ));
	printf("HMAC sha1   %s\n", bin2hex($sha1  ));
	printf("HMAC sha256 %s\n", bin2hex($sha256));
	printf("HMAC sha512 %s\n", bin2hex($sha512));

$md5    = hash_hmac('md5'   , $v, $k);
$sha1   = hash_hmac('sha1'  , $v, $k);
$sha256 = hash_hmac('sha256', $v, $k);
$sha512 = hash_hmac('sha512', $v, $k);
	printf("HMAC  md5   %s\n", $md5   );
	printf("HMAC sha1   %s\n", $sha1  );
	printf("HMAC sha256 %s\n", $sha256);
	printf("HMAC sha512 %s\n", $sha512);
