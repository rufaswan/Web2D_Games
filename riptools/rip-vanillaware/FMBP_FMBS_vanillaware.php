...
//   c4 = RGBA color
//////////////////////////////
function FMBS_s0( int $id ) : array
{
	tool::trace(__FUNCTION__, $id);
	global $gp_share;
	...
	for ( $i=0; $i < $sc; $i++ )
	{
		//echo '0';
		$p = $sp + ($i * $sk);
		$s = tool::substr($gp_share['file'], $p, $sk);
...
function FMBS_s0s1s2( int $id ) : array
{
	tool::trace(__FUNCTION__, $id);
	global $gp_share;
...
function FMBS_sb( int $id ) : array
{
	...
	{
		tool::trace('has no sb.txt', $gp_share['tag']);
		return [];
	}
		...
		case 'FMBS':
			...
			switch ( $ver )
			{
				...
				case 0x72:
					...
					{
						$poss = ['ps4_odin' , 'ps4_sent'];
						tool::warning('unable to auto-detect', implode(' , ', $poss));
					}
					return '';
...
function vanilla( string $tag, string $fname ) : void
{
	tool::trace(__FUNCTION__, $tag, $fname);
	...
	if ( empty($gp_share['file']) )
		return;

	$t = detect_tag($gp_share['file']);
	tool::trace('[AUTO] detect_tag()', $t);
	if ( ! empty($t) )
		$tag = $t;
	tool::trace('tag', $tag);
	...
	tool::save("$fname.v55", $txt);
}

$tag = '';
for ( $i=1; $i < $argc; $i++ )
	vanilla($tag, $argv[$i]);
//tool::usage($argv, 'vanilla');
 ...
 *
 *    ps5_grim  2023  PS5   [YET] GrimGrimoire OnceMore
 *    ps5_unic  2024  PS5   [YET] Unicorn Overlord
 *    ps5_mura  2027  PS5   [YET] Muramasa Revenant Blade
 *    xbx_unic  2024  XBSX  [YET] Unicorn Overlord
 *    swi_mura  2027  Swit  [YET] Muramasa Revenant Blade
 *    sw2_mura  2027  Swi2  [YET] Muramasa Revenant Blade
 *    pc_mura   2027  PC    [YET] Muramasa Revenant Blade
 *
 * @arg   $game  $list
 * @arg   FILE    $input
 */
