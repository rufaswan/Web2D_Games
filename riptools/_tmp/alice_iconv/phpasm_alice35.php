...
function sco_cmd( $ck, $cv )
{
	...
	switch ( $cv[2] )
	{
		...
		case '\\': // label call
			...
			$t2 = $arg[0];
			if ( $arg[0] )
				$gp_php[$ck] = FUNC_CMD . "( 'call' , [ $t1 , $t2 ] );";
			else
				$gp_php[$ck] = FUNC_CMD . "( 'call' , [ 0 , 0 ] );";
			return;
		case '%': // page call
			$id = calli( $arg[0] );
			$gp_php[$ck] = FUNC_CMD . "( 'call' , [ $id , 0 ] );";
			return;
		case '&': // page jump
			$id = calli( $arg[0] );
			$gp_php[$ck] = FUNC_CMD . "( 'jump' , [ $id , 0 ] );";
			return;
		//////////////////////////////
		case "sel_st":
			$loc = $arg[0];
			$lno = findline("$sco,$loc", $ck);
			$gp_php[$ck] = FUNC_CMD . "( 'sel_st' , [ $lno ] );";
			return;
		case "sel_ed":
			$gp_php[$ck] = FUNC_CMD . "( 'sel_ed' , [] );";
			return;
		//////////////////////////////
		case "DF":
			$var = varid( $arg[0] );
		...
		case "GS": // extend => x,y,w,h
			...
			$gp_php[$ck] = FUNC_CMD . "( $cmd , [ $num , $var ] );";
			return;
