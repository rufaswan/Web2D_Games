	...
	while ( $st < $ed )
	{
		...
		switch ( $b1 )
		{
			...
			case 0x16:
				$vsz = intv( $file, $st );
				$st += $vsz;
				printf('rpg::system += %x', $vsz);
				break;
			case 0x15:  lmu_int_array('rpg::terms', $file, $st );     break;
			case 0x14:
				$vsz = intv( $file, $st );
				$st += $vsz;
				printf('rpg::chipset += %x', $vsz);
				break;
			case 0x13:
				$vsz = intv( $file, $st );
				$st += $vsz;
				printf('rpg::animation += %x', $vsz);
				break;
			...
			case 0x10:
				$vsz = intv( $file, $st );
				$st += $vsz;
				printf('rpg::terrain += %x', $vsz);
				break;
			case 0xf:   lmu_int_array('rpg::troop', $file, $st );     break;
			case 0xe:
				$vsz = intv( $file, $st );
				$st += $vsz;
				printf('rpg::enemy += %x', $vsz);
				break;
