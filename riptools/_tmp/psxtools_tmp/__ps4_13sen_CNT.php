...
function sect_image( $fp, $base, $pfx, $flg )
{
	...
	$ndblk = str2int($sub, 0x38, 4);
	$seed  = substr ($sub, 0x370, 0x10);

	//$sub = fp2str($fp, $base, $blksz*$ndblk);
	//save_file("$pfx/root.bin", $sub);
...
function pkgfile( $fname )
{
	...
	printf("PFS image = %x [%x]\n", $off, $siz);
	if ( filesize($fname) !== ($off+$siz) )
		return;

	sect_image($fp, $off, "$pfx/image", $flg);
	return;
}
*/
function pkgfile( $fname )
