...
$gp_goto = [
	0x05 => [1, 3, true], 0x06 => [1, 5, true],
	0x0a => [2, 4, true],

	0x01 => [1, 3, false], 0x02 => [6, 8, false],
	0x84 => [3, 5, false], 0x85 => [3, 5, false], 0x86 => [3, 5, false],
	0x89 => [4, 6, false], 0x8a => [2, 4, false], 0x8b => [3, 5, false],
	0x8e => [5, 7, false],
	0x91 => [2, 4, false],
	0xb9 => [2, 4, false],
	0xc9 => [2, 4, false],
	0xcb => [2, 4, false],
	0xfb => [3, 5, false],
	0xfe05 => [4, 6, false), 0xfe06 => [4, 6, false],

	0x31 => [3, 5, false], 0x32 => [3, 5, false],
	0xe2 => [3, 5, false], 0xe3 => [3, 5, false],
	0xfe30 => [3, 5, false], 0xfe31 => [3, 5, false], 0xfe32 => [3, 5, false], 0xfe33 => [3, 5, false],
	0xfe34 => [4, 6, false], 0xfe35 => [4, 6, false], 0xfe36 => [4, 6, false], 0xfe37 => [4, 6, false],
];

function map5_label( string &$file ) : array
{
	$action = [
...
function map5_loclabel( string &$file, int &$pos, array &$label, array &$list, int $oppos, int $opsiz, bool $isfunc ) : void
{
	$goto = tool::ordstr($file, $pos + $oppos, 2);
	if ( $isfunc )
...
function map5_opcode( array &$oplist, string &$file, array &$label, int $pos, int $len ) : int
{
...
//////////////////////////////
function xeno_map5op( string &$file ) : void
{
	$cnt = tool::ordstr($file, 0x80, 2);

	$sub = substr($file, 0x84, $cnt*0x40);
	$pos = tool::ordstr($sub, 0, 2);
	...
	if ( $range[0] !== $pos )
	{
		...
		if ( $res !== $pos )
			return tool::warning('subfunc() before obj_0::init do not stop @ obj_0::init !');
		ksort($oplist);
	}
