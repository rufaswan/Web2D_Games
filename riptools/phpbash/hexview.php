...
//////////////////////////////
function term_title( array &$fps ) : string
{
	$name = [];
	foreach ( $fps as $v )
		$name[] = $v[0];
...
function term_opt( int $hx, int $cnt_fps ) : array
{
	...
	// STDIN on newline
	return [$tx, $ty-1, $byread];
}

function fp_read_hex( $fp, int $off, int $len ) : string
{
	$txt = ' |';
...
function hexview() : void
{
	...
	$hx = hexnum::hexdigit($gp_opt['size']);
	$is_done = false;
...
//////////////////////////////
function parse_options( array $argv, int $i ) : int
{
	global $gp_fps, $gp_opt;
	switch ( $argv[$i] )
	{
