...
//////////////////////////////
function png_unfilter( string &$idat, int $w, int $h, int $byte ) : void
{
	echo "== png_unfilter( $w , $h , $byte )\n";
	$rows = [];
	$dpw = $w * $byte;
	for ( $y=0; $y < $h; $y++ )
		$rows[] = tool::substr($idat, $y*($dpw+1), $dpw+1);
...
function png_8bpp( string &$idat, int $dp ) : string
{
	tool::trace(__FUNCTION__, $dp);
	switch ( $dp )
	{
		case 8:
			return $idat;
...
function png_plte( array &$chunk ) : string
{
	if ( ! isset( $chunk['PLTE'] ) )
		return '';
	echo "== png_plte()\n";
...
//////////////////////////////
function png2clut( array &$chunk, int $w, int $h, int $dp, int $cl, string $fname ) : void
{
	tool::trace(__FUNCTION__, $w , $h , $dp , $cl , $fname);
...
function png2rgba( array &$chunk, int $w, int $h, int $dp, inr $cl, string $fname ) : void
{
	tool::trace(__FUNCTION__, $w , $h , $dp , $cl , $fname);
	...
	while ( $i < $sz )
	{
		$rgba .= $idat[$i+0] . $idat[$i+1] . $idat[$i+2];
		if ( $cl & 4 ) // is RGBA
		{
			$rgba .= $idat[$i+3];
			$i += 4;
...
//////////////////////////////
function png_chunks( string &$png ) : array
{
	$chunk = [];
	...
	while ( $st < $ed )
	{
		...
		$len = ieee754::ordstr($png, $st+0, 4);
		tool::trace($st, $len, $mgc);

		$dat = tool::substr($png, $st+8, $len);
		if ( ! isset( $chunk[$mgc] ) )
			$chunk[$mgc] = '';
		$chunk[$mgc] .= $dat;
...
function png2img( string $fname ) : int
{
	$png = file_get_contents($fname);
	if ( empty($png) )  return -1;

	if ( substr($png,0,8) !== PNG_MAGIC )
		return -1;

	$chunk = png_chunks($png);
	$ihdr  = &$chunk['IHDR'];
	$w = ieee754::ordstr($ihdr, 0, 4);
	$h = ieee754::ordstr($ihdr, 4, 4);
	$dp = ord($ihdr[ 8]); // bit depth
	$cl = ord($ihdr[ 9]); // color type , +1=index  +2=rgb  +4=alpha  (invalid=1 1+4 1+2+4)
	$cm = ord($ihdr[10]); // compression , 0=zlib
	$fl = ord($ihdr[11]); // filter , 0=adaptive/5 type
	$in = ord($ihdr[12]); // interlace , 0=none , 1=adam7

	if (  $dp >  8 )
		return tool::warning('bit depth >8 not supported');
	if ( ($cl &  2) == 0 )
		return tool::warning('grayscale not supported');
	if (  $in !== 0 )
		return tool::warning('adam7 interlace not supported');
...
tool::argv_callback($argv, 'png2img');
