...
require 'tool.inc';
tool::require('class-html');

function strip_dom( array &$dom, string $tag ) : void
{
	$c = count($dom);
	while ( $c > 0 )
	{
...
function strip_dom_nest( array &$dom, string $tag ) : void
{
	$is_nest = true;
	$stpos = -1;
	while ( $is_nest )
...
function strip_attrib( string &$tag ) : void
{
	if ( empty($tag) )
		return;
	if ( $tag[0] !== '<' )
...
function htmlclean( string $fname ) : void
{
