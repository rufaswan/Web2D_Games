...
function J_cmd35( &$file, &$st, &$run )
{
	...
	switch( $type )
	{
		// J0 x,y:        次に来るＧコマンドでのＣＧ表示開始座標を指定する (絶対座標指定)
		// J1 x,y:        次に来るＧコマンドでのＣＧ表示開始座標を指定する (相対指定)
		case 0:
		case 1:
			$st += 2;
...
function C_cmd35( &$file, &$st, &$run )
{
	...
	switch( $file[$st+1] )
	{
		...
		case 'B':
			...
			$gp_pc["CB"] = array($start_x , $start_y , $lengs_x , $lengs_y , $color);
			return;
		...
		case 'K':
			...
			switch ( $type )
			{
				...
				case 3:
					...
					trace("CK 3 , $x0 , $y0 , $cx , $cy , $dst , $src , $count , $z1");
					return;
...
// GX cg_no,shadow_no:(24bitDIB only)    影データを指定してＣＧをロードする
function G_cmd35( &$file, &$st, &$run )
{
	global $gp_pc, $gp_img_meta;
	switch ( $file[$st+1] )
	{
		// GS num,var:        num 番にリンクされているＣＧの座標とサイズを取得する
		case 'S':
			$st += 2;
			$num = sco35_calli($file, $st);
...
// LC x,y,ﾌｧｲﾙ名:        ＣＧを表示する。
// LH? 1,no:        ＣＤのデータをＨＤＤへ登録する
// LH? 2,no:        ＣＤのデータをＨＤＤへ削除する
...
function V_cmd35( &$file, &$st, &$run )
{
	...
	switch( $file[$st+1] )
	{
		...
		case 'H':
			...
			return;
		// VP nPage,x0Unit,y0Unit,nxUnit,nyUnit,bSpCol:        ユニットＣＧ取得位置＆並び状態設定＆スプライト色指定 （転送元設定）
		case 'P':
			$st += 2;
			$nPage  = sco35_calli($file, $st);
...
function Z_cmd35( &$file, &$st, &$run )
{
	...
	switch( $file[$st+1] )
	{
		...
		case 'F':
			...
			$gp_pc["ZF"] = $sw;
			return;
		// ZG var:        ＣＧのロードした回数をリンク番号毎に配列に書き込む配列を設定
		case 'G':
			$st += 2;
		...
		case 'H':
			...
			$gp_pc["ZH"] = $switch;
			return;
		// ZI key,mode:    Ａコマンドのキー入力待ち時の各キーの動作の指定
		case 'I':
			$st += 2;
	...
	switch( $file[$st] )
	{
		...
		case '{': // 0x7b
			...
			if ( ! $exp )
				$gp_pc["pc"][1] = $end;
			return;
		// < var,start,end,sign,step:        < ＦＯＲループ開始
		case '<': // 0x3c
			$type = ord( $file[$st+1] );
			$st += 2;
			if ( $type == 0 )
