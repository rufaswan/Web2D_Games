...
In this case, we found Bart's HP is at `RAM 800ccd34` duing battle.
...
.Put a WRITE breakpoint there, you'll get this on Margie's turn.
image::margie-heal-1.png[]

TIP: Use the register list on top-right for keep track its value. For example, `s3` is `0x800c4078`
...
image::margie-heal-2.png[]

The value is copied from `t0`.



.Again, load back #BEFORE TURN state# and put a WRITE breakpoint on `RAM 800d2c54` to backtrace it.
image::margie-heal-3.png[]
...
image::margie-heal-4a.png[]

IMPORTANT: Bookmark this ASM address!

Following `v1`, we follow the pointer at `a0`, and reached `RAM 800cd138 + 5b`, we get the value 2.

Looking at the data struct, it seems like character stats. Margie has Ether = 2.

TIP: The table of hex number below ASM code is the RAM! All in-game variable can be found here!.

NOTE: Game Genie and Action Replay is about is about constantly modifying the values here for infinite money, infinite health, infinite ammo, etc.



.Following `v0`, we follow the pointer at `v0`, and reached `RAM 800d0388 + 11`, we get the value 0x14.
image::margie-heal-4b.png[]

#IT FITS!# 2 * 0x14 = 0x28. So the formula is Margie's Ether 2 * Spell Power 0x14 = 0x28 HP Heal!

Now we are getting somewhere!

Let's load back #BEFORE TURN state# and put a WRITE breakpoint here to see where 0x14 came from!

Unfortunately the breakpoint didn't triggered. It means the data wasn't update during the battle.

It also means we need to go further back



.Load back #BEFORE BATTLE state# and re-trigger the battle from the start.
image::margie-heal-5b.png[]
...
Let's copy a line of compressed data to find the correct game file from the disc.

```
a1 28 70 01  04 10 05 10  00 12 fd 28  f0 a8 c8 30
```
...
Xenogears looks for its game files by LBA (Logical Block Address) or CD sector address. The list of LBA for the whole game is on SLPS/SLUS main executable at offset 0x804 to 0x8800. It is then loaded to RAM at 0x80010004 to 0x80018000.



.So we put an READ breakpoint to the entire area.
image::xeno-file-1.png[]
...
.When the game stops at 3,4,5,6
image::xeno-file-2.png[]
...
.When the game stops at 0,1,2
image::xeno-file-3.png[]
...
image:heal-200-margie.png[]
image:heal-200-miang.png[]
