...
image::spec/spec-rel.png[spec/spec-rel.png]
...
----
{
  ...
  "color"          : string rgba,
  "matrix_mix_id"  : int id,
  "color_mix_id"   : int id,
  "dstquad_mix_id" : int id,
  "fogquad_mix_id" : int id,
  "srcquad_mix_id" : int id,
  "hitquad_mix_id" : int id,
}
----
...
time (required)::
	* Measured in FPS (Frames Per Seconds).
	* 1 second is 1000 milliseconds.
	* Standards
	** Hand drawn 2D animation is 24 FPS
	*** on ones (24 FPS) is 41.67 milliseconds.
	*** on twos (12 FPS) is 83.33 milliseconds.
	** NTSC is 60 FPS
	*** 60 FPS is 16.67 milliseconds.
	*** 30 FPS is 33.33 milliseconds.
	** PAL is 50 FPS
	*** 50 FPS is 20 milliseconds.
	*** 25 FPS is 40 milliseconds.
...
matrix_mix_id::
	* Index of <<mix_object>> to generate in-betweens for 2 or more matrix.
	* If omitted or <<mix_object>> is invalid, then it is  default to -1.
	* -1 means no mixing. Only current matrix is used.

color_mix_id::
	* Index of <<mix_object>> to generate in-betweens for 2 or more color.
	* If omitted or <<mix_object>> is invalid, then it is  default to -1.
	* -1 means no mixing. Only current color is used.

dstquad_mix_id , srcquad_mix_id and fogquad_mix_id::
	* Index of <<mix_object>> to generate in-betweens for 2 or more keyframes.
	* Affects `dstquad`, `srcquad` and `fogquad` respectively, from attached <<keyframe_layer_object>>.
	* If omitted or <<mix_object>> is invalid, then it is  default to -1.
	** Mixing will also default to -1 when all attached keyframe are:
	*** does not have the same layer count.
	*** one of the keyframe is empty.
	*** one of the keyframe has empty quad
	* -1 means no mixing. Only current quad is used.

hitquad_mix_id::
	* Index of <<mix_object>> to generate in-betweens for 2 or more hitbox.
	* Affects `hitquad` respectively, from attached <<hitbox_layer_object>>.
	* If omitted or <<mix_object>> is invalid, then it is  default to -1.
	** Mixing will also default to -1 when all attached hitbox are:
	*** does not have the same layer count.
	*** one of the hitbox is empty.
	*** one of the hitbox has empty quad
	* -1 means no mixing. Only current quad is used.
...
v0.6 (2025-12-20)::
	...
	* Added `mix` objects
	* `animation_timeline_object` : replaced `keyframe_mix` with `dstquad_mix_id`, `fogquad_mix_id` and `srcquad_mix_id`
	* `animation_timeline_object` : replaced `hitbox_mix`   with `hitquad_mix_id`
	* `animation_timeline_object` : replaced `matrix_mix`   with `matrix_mix_id`
	* `animation_timeline_object` : replaced `color_mix`    with `color_mix_id`
...
* link:spec/sample-v0.4-keyframe-order.quad[]
* link:spec/sample-v0.5-keyframe-attribute.quad[]
* link:spec/sample-v0.5-keyframe-colorize.quad[]
* link:spec/sample-v0.6-animation-matrix-mix.quad[]
* link:spec/sample-v0.6-animation-keyframe-mix.quad[]
