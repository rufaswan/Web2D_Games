...
mode_rgb (required)::
	...
	** `mode` is enum for `blendEquation()`
	...
	** `src_factor` and `dst_factor` are enum for `blendFunc()`
	*** Static value
	**** `ZERO` is value `0.0`
	**** `ONE` is value `1.0`
...
logic_op::
	* Can only be used for integer draw buffer. It has no effect for floating-point draw buffer.
	* When it is set, normal blending is disabled. Hence `mode_rgb`, `mode_alpha` and `color` are ignored.
	* Accepts 4 strings array for each RGBA components.
	** Can be shorthand to 1 string when all 4 RGBA components are the same.
	** `opcode` is enum for `logicOp()`
	*** `CLEAR` as value 0
	*** `SET` as value 1
...
v0.x::
	...
	* `blend_object` : removed `mode`
	* `blend_object` : added `mode_rgb`
	* `blend_object` : added `mode_alpha`
...
Canvas XY::
	...
	* For Canvas size 320x240, X is -160px to +160px , and Y is -120px to +120px.
	* It is then normalized to -1.0 to +1.0.

Texture UV::
	* +X is right direction, +Y is down direction, and they are measured in px (pixels).
	* 0,0 is at top-left of texture.
	* For Texture size 320x240, X is 0px to 320px , and Y is 0px to 240px.
