...
keyframe_mix::
	* Marks if current keyframe layer dstquad is interpolated with the next keyframe layer dstquad.
	** rate = t / time , t++
	** dstquad = (current * (1 - rate)) + (next * rate)
	* Mixing is impossible under the following conditions, and this option is ignored.
	** if current keyframe layer count is not the same as next keyframe layer count. Default to use current keyframe layer only.
	** if current keyframe layer is empty. Default to skipped.
	** if next keyframe layer is empty. Default to use current keyframe layer only.
	* If omitted, then it is default to `0` (`false`)

hitbox_mix::
	* Marks if current hitbox layer hitquad is interpolated with the next hitbox layer hitquad.
	** rate = t / time , t++
	** hitquad = (current * (1 - rate)) + (next * rate)
	* Mixing is impossible under the following conditions, and this option is ignored.
	** if current hitbox layer count is not the same as next hitbox layer count. Default to use current hitbox layer only.
	** if current hitbox layer is empty. Default to skipped.
	** if next hitbox layer is empty. Default to use current hitbox layer only.
	* If omitted, then it is default to `0` (`false`)
...
v0.2 (2023-05-11)::
	...
	* `animation_timeline_object`: added `mix`
