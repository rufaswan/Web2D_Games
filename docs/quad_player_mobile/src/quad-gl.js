function QuadGL(Q){
	...
	$.init = function( dom ){
		...
		vert_src = `
			...
			void main(void){
				...
				XYZ.x = (XYZ.x + 0.5) * u_pxsize[0].x;
				XYZ.y = (XYZ.y + 0.5) * u_pxsize[0].y;

				v_fog = FOG;
				v_uv  = UV;
				v_z   = z;
				gl_Position = vec4(XYZ.x, XYZ.y, a_z, 1.0);
			}
		`;
		frag_src = `
			...
			void main(void){
				...
				FOG = v_fog * z;
				UV  = v_uv  * z;
				gl_FragColor = texture2D(u_tex, UV * u_pxsize[1]) * FOG;
	...
	$.drawVRAM = function( vram, tex, rect ){
		...
		var pxsz = [ 1.0/view[0] , 1.0/view[1] , 1.0/sw , 1.0/sh ];
		__.GL.activeTexture(__.GL.TEXTURE0);
		__.GL.bindTexture  (__.GL.TEXTURE_2D, tex);

		var rx1 = rect[0] - view[0];
		var ry1 = rect[1] - view[1];
		var rx2 = rect[2] - view[0];
		var ry2 = rect[3] - view[1];
		var dst = [rx1,ry1 , rx2,ry1 , rx2,ry2 , rx1,ry2];
